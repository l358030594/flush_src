...
#define UDISK_4K_SECTOR_READ_BUFFERLESS  1
...
struct mass_storage {
    ...
    u8 *udisk_cache_buf;            //用于文件系统数据区
    u32 align_start_lba_for_lut;    //用于文件系统查找表(e.g. FAT表，MBR)
    u32 align_end_lba_for_lut;      //用于文件系统查找表(e.g. FAT表，MBR)
    u8 *udisk_cache_buf_for_lut;    //用于文件系统查找表(e.g. FAT表，MBR)
    u32 fs_fatbase[4];              //文件系统内FAT表区起始地址
    u32 fs_database[4];             //文件系统内数据区起始地址
#endif

...
u32 mtp_register(const usb_dev id);
u32 mtp_release();
...
struct mtp_command_block {
    uint32_t container_length;   //总长度
    uint16_t container_type;     // 类型
    uint16_t code;               // 命令代码
    uint32_t transaction_id;     // 事务ID
    uint32_t parameter1;         // 参数1
    // uint32_t parameter2;         // 参数2
    // uint32_t parameter3;         // 参数3
    // uint32_t parameter4;         // 参数4
    // uint32_t parameter5;         // 参数5
};
struct mtp_data_block {
    uint32_t container_length;   //总长度
    uint16_t container_type;     // 类型
    uint16_t code;               // 响应状态代码
    uint32_t transaction_id;     // 事务ID，匹配请求
...
struct mtp_response_block {
    ...
    uint16_t code;               // 响应状态代码
    uint32_t transaction_id;     // 事务ID，匹配请求
    // uint32_t parameter1;         // 参数1，可能包含额外信息
    // uint32_t parameter2;         // 参数2
    // uint32_t parameter3;         // 参数3
    // uint32_t parameter4;         // 参数4
    // uint32_t parameter5;         // 参数5
};
struct mtp_event_block {
    uint32_t container_length;   //总长度
    uint16_t container_type;     // 类型
    uint32_t event_code;         // 事件代码
    uint32_t transaction_id;     // 事务ID
    // uint32_t parameter1;         // 参数1，具体事件相关
    // uint32_t parameter2;         // 参数2
    // uint32_t data_length;        // 事件数据长度
    // uint8_t* data;               // 事件数据指针
};
// // 定义 GetDeviceInfo 返回的结构体
// typedef struct {
//     uint16_t standardVersion;       // 标准版本
//     uint32_t vendorExtensionID;     // 厂商扩展 ID
//     uint16_t vendorExtensionVersion;// 厂商扩展版本
//     char vendorExtensionDesc[256];  // 厂商扩展描述
//     uint16_t functionalMode;        // 功能模式
//     uint16_t operationsSupported[10];// 支持的操作（最多10个）
//     uint16_t eventsSupported[10];   // 支持的事件（最多10个）
//     uint16_t devicePropertiesSupported[10]; // 支持的设备属性（最多10个）
//     uint16_t captureFormats[10];    // 捕获格式（最多10个）
//     uint16_t playbackFormats[10];   // 回放格式（最多10个）
//     char manufacturer[256];         // 制造商
//     char model[256];                // 型号
//     char deviceVersion[256];        // 设备版本
//     char serialNumber[256];         // 序列号
// } MTPDeviceInfo;

struct mtp_block_header {
    uint32_t container_length;   //总长度
    uint16_t container_type;     // 类型
    uint16_t code;               // 命令代码
    uint32_t transaction_id;     // 事务ID
...
#define MAX_MTP_DEV         2
#define MTP_DEV_NAME_LEN    14
struct mtp_info {
    void (*mtp_wakeup_handle)(struct usb_device_t *usb_device);
    void *dev_handle[MAX_MTP_DEV];
    char dev_name[MAX_MTP_DEV][MTP_DEV_NAME_LEN];
    struct __dev *dev;
    struct vfscan   *fsn;
};

struct usb_mtp_handle {
    struct mtp_command_block command;
    struct mtp_data_block data;
    struct mtp_response_block response;
    struct mtp_event_block event;
    // struct mtp_block_header block_header;
...
#define MTP_BLOCK_HEADER_SIZE   12

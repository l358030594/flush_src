...
#define SOURCE_DEV4_MSBC_TEST_ENABLE	0		//MSBC解码测试, 使用MSBC固定数据测试
...
#if SOURCE_DEV4_MSBC_TEST_ENABLE
...
/*
   输入源节点数据
   	...
   	*len 源节点数据长度
   	return 源节点数据指针
*/
static u8 *source_dev4_get_packet(struct source_dev4_file_hdl *hdl, u32 *len)
{
    u8 *packet = NULL;
    u32 packet_len = 0;
...
//释放源节点数据
static int source_dev4_free_packet(struct source_dev4_file_hdl *hdl, u8 *packet)
{
    /* free(packet); */
    return 0;
}
...
*/
static void source_dev4_get_fmt(struct source_dev4_file_hdl *hdl, struct stream_fmt *fmt)
{
#if SOURCE_DEV4_MSBC_TEST_ENABLE
...
static enum stream_node_state source_dev4_get_frame(void *_hdl, struct stream_frame **_frame)
{
    ...
    //1、获取当前节点数据
    u8 *packet = source_dev4_get_packet(hdl, &len);
    if (!packet) {
        *_frame = NULL;
        //表示源节点获取不到数据
    ...
    //4、释放当前节点数据
    source_dev4_free_packet(hdl, packet);
...
//获取当前蓝牙地址
static int source_dev4_ioc_set_bt_addr(struct source_dev4_file_hdl *hdl, u8 *bt_addr)
{
...
//获取当前节点数据参数
static void source_dev4_ioc_get_fmt(struct source_dev4_file_hdl *hdl, struct stream_fmt *fmt)
{
    source_dev4_get_fmt(hdl, fmt);
}
...
static int source_dev4_ioctl(void *_hdl, int cmd, int arg)
{
    ...
    case NODE_IOC_SET_BTADDR:
        source_dev4_ioc_set_bt_addr(hdl, (u8 *)arg);
        break;
    case NODE_IOC_GET_FMT:
        source_dev4_ioc_get_fmt(hdl, (struct stream_fmt *)arg);
        break;

...
#pragma bss_seg(".tuya_ble_bulk_data.data.bss")
#pragma data_seg(".tuya_ble_bulk_data.data")
#pragma const_seg(".tuya_ble_bulk_data.text.const")
#pragma code_seg(".tuya_ble_bulk_data.text")
#endif
/**
...
uint32_t tuya_ble_bulk_data_read_block_size_get(void)
{
    ...
    if (tuya_ble_connect_status_get() != BONDING_CONN) {
        ...
        uint32_t mtu_length = tuya_ble_send_packet_data_length_get();
        if (mtu_length < 100) {
            read_block_size = 256;
...
void tuya_ble_handle_bulk_data_evt(tuya_ble_evt_param_t *evt)
{
    ...
    if (data_length > 0) {
        if (evt->bulk_res_data.evt == TUYA_BLE_BULK_DATA_EVT_READ_BLOCK) {
            ...
            tuya_ble_commData_send(bulk_data_cmd, 0, p_buf, data_length, encry_mode);
            tuya_ble_free(p_buf);
        } else {

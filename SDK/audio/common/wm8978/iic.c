...
#pragma bss_seg(".iic.data.bss")
#pragma data_seg(".iic.data")
#pragma const_seg(".iic.text.const")
...
/** @brief: IIC 模块初始化函数
    ...
    @return:void
    @author:Juntham
    @note:  void wm8978_iic_init_io(void)
*/
/*----------------------------------------------------------------------------*/
...
/**@brief   STOP IIC
   @param   无
   @return  无
   @note    void wm8978_iic_stop(void)
*/
/*----------------------------------------------------------------------------*/
...
/**@brief   向IIC总线发送一个BYTE的数据
   ...
   @return  无
   @note    void wm8978_iic_sendbyte_io(u8 byte)
*/
/*----------------------------------------------------------------------------*/
...
static bool g_iic_busy = 0; ///<iic繁忙标记
/*----------------------------------------------------------------------------*/
/** @brief: IIC 模块初始化函数
    @param: void
    @return:void
...
{
    g_iic_busy  = 1;
    wm8978_iic_start();                //I2C启动
    iic_sendbyte(iic_dat);      //写数据
    wm8978_iic_stop();                 //I2C停止时序
}
/*----------------------------------------------------------------------------*/
/**@brief   IIC写函数
...
*/
/*----------------------------------------------------------------------------*/
void wm8978_eeprom_page_write_start(void)
{
    g_iic_busy  = 1;
    wm8978_iic_start();                //I2C启动
...
*/
/*----------------------------------------------------------------------------*/
void wm8978_eeprom_page_write_stop(void)
{
    wm8978_iic_stop();                     //I2C停止iic_data_out
    g_iic_busy = 0;
}
/*----------------------------------------------------------------------------*/
/**@brief   EEROM读函数
...
*/
/*----------------------------------------------------------------------------*/
tu8 wm8978_read_eerom(u8 iic_addr)
{
    u8  tbyte;
    g_iic_busy = 1;
    wm8978_iic_start();                    //I2C启动
    iic_sendbyte(0xa0);             //写命令
    iic_sendbyte(iic_addr);       	//写地址
    wm8978_iic_start();                    //写转为读命令，需要再次启动I2C
    iic_sendbyte(0xa1);             //读命令
    tbyte = iic_revbyte(1);
    wm8978_iic_stop();                     //I2C停止
    g_iic_busy = 0;
    return  tbyte;
}
...
*/
/*----------------------------------------------------------------------------*/
void wm8978_write_eerom(u8 addr, u8 dat)
{
...
*/
/*----------------------------------------------------------------------------*/
void wm8978_set_memory(u8 addr, u8 dat)
{
    write_eerom(addr, dat);
}
/*----------------------------------------------------------------------------*/

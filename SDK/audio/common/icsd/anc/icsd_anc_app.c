...
#define icsd_anc_log(...)
	...
	(ICSD_ANC_MODE == TWS_HALF_INEAR_MODE) || \
...
u8 ICSD_ANC_DMA_DOUBLE = 1;
u8 ICSD_ANC_TOOL_DATA_EN = 1;
const u8 ICSD_ANC_BYPASS_ON_FIRST = 1;//播放提示音过程中打开BYPASS节省BYPASS稳定时间

u32 train_time;
__anc_tool_ram SD_TRAM;
...
u8	ICSD_ANC_PARAM_DEBUG = 0;

u16 DEBUG_ANC_EN = DEBUG_ANC_DATA;
...
{
    float pow10n = exp_float((float)n / v1_log10_anc(exp_float((float)1.0)));
    return pow10n;
}
...
void icsd_anc_dma_on(u8 out_sel, int *buf, int len)
{
    anc_dma_on(out_sel, buf, len);
}

void icsd_anc_double_dma_on(u8 out_sel, int *buf, int len, u8 *cnt)
{
    printf("icsd_anc_double_dma_on\n");
    *cnt = ANC_DMA_DOUBLE_CNT + 1;
    anc_dma_on_double(out_sel, buf, len);
}

float icsd_anc_readfbgain()
{
...
void icsd_anc_dma_done()
{
    if (ICSD_ANC.adaptive_run_busy) {
        if (user_train_state & ANC_USER_TRAIN_DMA_EN) {
            if (ICSD_ANC_DMA_DOUBLE) {

#if ICSD_ANC_VER == ANC_VER_2
                icsd_anc_dma_double_done();
#endif
...
void icsd_anc_init(audio_anc_t *param, u8 mode, u8 seq, int tone_delay, u8 tws_balance_en)
{
    ...
    //icsd_debug_anc_en();
    anc_printf = _anc_printf;
    printf("icsd_anc_init\n");

    sd_anc_init();

#if ICSD_ANC_VER == ANC_VER_2
    if (ICSD_ANC_TOOL_DEBUG) {
        __icsd_anc_config config;
        config.type = 0;
        config.mode = 0;
        u8 button = test_run;
        icsd_anc_tool_debug_init(&config, button);
    }
#endif
    ...
    ANC_BOARD_PARAM->ear_canal_mem_data = (__ear_canal_mem_data *)ear_canal_mem_data;
    ANC_BOARD_PARAM->ear_canal_mem_cur_num = ear_canal_mem_cur_num;
...
void icsd_anctone_dacon_handler()
{
    ...
    u32 sample_rate = icsd_sample_rate;
    if (icsd_anc_contral & ICSD_ANC_FORCED_EXIT) {
        return;
    }
    if (user_train_state & ANC_TRAIN_TONE_PLAY) {
        ...
        if (user_train_state & ANC_TONE_ANC_READY) {
            ...
            case TWS_HALF_INEAR_MODE:
                sys_hi_timeout_add((void *)icsd_anc_id, icsd_anc_tonel_start, slience_ms + HALF_INEAR_TONE_DELAY);
                sys_hi_timeout_add((void *)icsd_anc_id, icsd_anc_pzl_start, slience_ms + HALF_INEAR_PZ_DELAY);
                break;
...
void icsd_anctone_dacon(u32 slience_frames, u32 sample_rate)
{
...
#if ICSD_ANC_SZ_OUT_EN

extern float icsd_anc_sz_data[TARLEN + TARLEN_L];
extern float icsd_anc_pz_data[TARLEN + TARLEN_L];
extern u8 sz_out_exist;

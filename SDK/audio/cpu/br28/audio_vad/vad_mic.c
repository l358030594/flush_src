...
#pragma bss_seg(".vad_mic.data.bss")
#pragma data_seg(".vad_mic.data")
#pragma const_seg(".vad_mic.text.const")
#pragma code_seg(".vad_mic.text")
#endif
/*****************************************************************
>file name : vad_mic.c
...
static void lp_vad_mic_in_enable(struct vad_mic_platform_data *data)
{
    ...
    gpio_set_mode(IO_PORT_SPILT(IO_PORTA_01), PORT_INPUT_FLOATING);
    if (data->mic_data.mic_mode == AUDIO_MIC_CAP_DIFF_MODE || data->mic_data.mic_bias_inside) {
        gpio_set_mode(IO_PORT_SPILT(IO_PORTA_02), PORT_INPUT_FLOATING);
    }
}
